...
type View = "dashboard" | "accounts" | "system" | "organizations" | "models";
...
export function AdminShell({ token, onLogout }: { token: string; onLogout: () => void }) {
  ...
  return <div className="admin-shell">
    <aside className="admin-sidebar"><div className="admin-brand"><span className="admin-brand-mark"><ShieldCheck aria-hidden="true" /></span><span><strong>{text[RSC.ADMIN_BRAND_TEXT]}</strong><small>v{adminPackage.version}</small></span><LanguageSwitcher text={text} language={language} /></div><p className="admin-nav-label">{text[RSC.ADMIN_NAV_SECTION]}</p><nav aria-label={text[RSC.ADMIN_NAV_SECTION]}>{nav.map(({ id, icon: Icon, label }) => <button className={`admin-nav-item ${activeView === id ? "is-active" : ""}`} key={id} onClick={() => setActiveView(id)}><Icon aria-hidden="true" /><span>{label}</span></button>)}</nav><div className="admin-sidebar-footer"><span className="admin-status-dot" aria-hidden="true" />{text[RSC.ADMIN_BADGE_TEXT]}<button className="admin-logout" onClick={onLogout}><LogOut aria-hidden="true" />{text[RSC.AUTH_LOGOUT_BUTTON]}</button></div></aside>
    <main className="admin-main"><div className="admin-content">{activeView !== "organizations" && activeView !== "models" && <div className="page-heading"><div><div className="eyebrow">{text[RSC.ADMIN_BADGE_TEXT]}</div><h1>{activeView === "dashboard" ? text[RSC.ADMIN_OVERVIEW_TITLE] : activeView === "system" ? text[RSC.ADMIN_SYSTEM_TITLE] : text[RSC.ADMIN_NAV_ACCOUNTS]}</h1></div></div>}{activeView === "dashboard" ? <DashboardScreen text={text} /> : activeView === "system" ? <SystemScreen text={text} /> : activeView === "organizations" ? <OrganizationScreen token={token} request={api} /> : activeView === "models" ? <ModelsScreen token={token} request={api} /> : <AccountScreen token={token} request={api} />}</div></main>
  </div>;
}
