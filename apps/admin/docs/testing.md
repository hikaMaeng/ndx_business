...
node creation is absent from the node modal. The model tab picks one model from
every registered model, checks the choice replaces rather than adds, clears it,
and checks a child with nothing set names the ancestor it inherits from.
