...
- standby Gateway는 ownership 대기 중 `/health`만 제공하고, ownership 뒤에는 같은 bound HTTP server의 request handler를 active Gateway handler로 교체한다. 이유: retention 초기화 시간과 무관하게 host port liveness를 유지한다.
