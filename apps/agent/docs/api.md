...
`GET /metrics` returns aggregate ingress, append, and worker counters as `{ service, metrics }`. It is registered before the SPA fallback, so a missing route no longer answers `200` with `index.html`. `AGENT_METRICS_TOKEN` is required: an unset token disables the route with `404`, and a request without `Authorization: Bearer <token>` is rejected with `401`. The snapshot carries counts and latency totals only; payloads, channels, and session identifiers are never returned.
