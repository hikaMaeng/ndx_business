...
Run `npm test --workspace agent_app`, `npm test --workspace agent_domain`, both lint commands, and `npm run deploy agent`. Submit a unique `attempt-ledger-proof-*` transaction and inspect its `event_processing_attempt` row for `worker_id`, `started_at`, `finished_at`, and a null `lease_until`; delete only that transaction's rows and result message afterward.
