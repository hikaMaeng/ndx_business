...
Run `npm test --workspace agent_app`, `npm test --workspace agent_domain`, `npm run lint --workspace agent_app`, then `npm run deploy agent`. The deploy must report the published health probe as HTTP 200. Use unique `domain-handler-proof-*` and `domain-abort-proof-*` transaction keys and remove only those scoped rows after the run.
