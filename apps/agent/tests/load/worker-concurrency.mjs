...
const streams = Number(process.env.AGENT_DELAY_STREAMS ?? Math.min(total, 512));
...
assert.ok(Number.isInteger(total) && total > 0); assert.ok(Number.isInteger(workers) && workers > 0); assert.ok(Number.isInteger(streams) && streams > 0 && streams <= total); assert.ok(delayMs >= 1_000);
...
await Promise.all(Array.from({ length: total }, (_, index) => fetch(`${baseUrl}/api/events`, {
  ...
  body: JSON.stringify({ action: "test.delay", transactionKey: `${prefix}-${index}`, channel: ["agent.requests", "orders", "telemetry", "notifications"][index % 4], replyChannel: "agent.results", payload: { simulateDelayMs: delayMs, sessionKey: `delay-session-${index % streams}` } }),
