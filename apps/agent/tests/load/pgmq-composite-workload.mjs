...
try {
  ...
  assert.equal(visibilitySeconds, deployed.visibilityTimeoutSeconds, "harness visibility setting must match the deployed Worker");
  assert.equal(executionLeaseSeconds, deployed.executionLeaseSeconds, "harness execution lease setting must match the deployed Worker");
  gatewayQueue = deployedGatewayQueue();
  const baseline = await metrics();
  assert.deepEqual(baseline.configuration, deployed, "Gateway metrics must report the deployed reliability configuration");
