...
test("a joined transaction preserves its command for lease-expiry recovery without repeating work", async () => {
  ...
  const loop = startWorkerConsumer({
    ...
    executions: { claim: async () => ({ kind: "joined", requestEventId: command.eventId, completed: false }), complete: async () => true, recipients: async () => [], renew: async () => true } as never,
...
test("a running worker renews both its execution and PGMQ visibility leases", async () => {
  ...
  const loop = startWorkerConsumer({
    ...
    commandQueue: "agent_commands", resultQueue: "agent_results", eventStore: { append: async (draft: EventDraft): Promise<EventEnvelope> => ({ ...draft, sequence: "1" }) } as never,
    executions: {
    ...
    metrics: { increment: () => undefined } as never, visibilitySeconds: 3, pollSeconds: 1, batchSize: 1, maxInFlight: 2,
  });
