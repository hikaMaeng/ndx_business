...
test("router fans one result into only the subscribed Gateway queues", async () => {
  let read = false; const sends: string[] = []; const deleted: string[] = [];
  const loop = startResultRouter({
    queue: { read: async () => read ? await new Promise<never>(() => undefined) : (read = true, [{ id: "42", event, headers: null }]), ensure: async () => undefined, send: async (queue: string) => { sends.push(queue); return "x"; }, delete: async (_queue: string, id: string) => { deleted.push(id); } } as never,
    ...
    metrics: { increment: () => undefined } as never, visibilitySeconds: 60, pollSeconds: 1,
  });
  await new Promise((resolve) => setTimeout(resolve, 15)); loop.stop();
  assert.deepEqual(sends, ["agent_gateway_gateway_a", "agent_gateway_gateway_b"]);
