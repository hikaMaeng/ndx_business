...
export function writeMetrics(request: IncomingMessage, response: ServerResponse, env: AgentEnv, metrics: MetricsRegistry): boolean {
  ...
  response.end(JSON.stringify({ service: "agent", configuration: {
    ...
    maxExecutionAttempts: env.maxExecutionAttempts, maxOutboxAttempts: env.maxOutboxAttempts, terminalPersistenceAlertAttempts: env.terminalPersistenceAlertAttempts, terminalPersistenceBackoffMaxSeconds: env.terminalPersistenceBackoffMaxSeconds,
