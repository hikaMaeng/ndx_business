...
import { createVibeApp } from "./app.js";
import { createSessionVerifier } from "./auth/index.js";
import { createVibeSocketPolicy } from "./socket-policy.js";
import {
...
if (env.role === "worker") {
  ...
  const verifier = createSessionVerifier({ adminBaseUrl: process.env.VIBE_ADMIN_BASE_URL ?? "http://admin:18080", cacheMs: Number(process.env.VIBE_SESSION_CACHE_MS ?? 5_000) });
  gatewayStandby.activate(createVibeApp({
    env, queue, metrics, verifier,
    checkDatabase: async () => { await Promise.all([queue.check(), database.query("SELECT 1")]); },
    frontDir,
    workspaceRoot: process.env.VIBE_WORKSPACE_ROOT ?? "/workspace",
    adminBaseUrl: process.env.VIBE_ADMIN_BASE_URL ?? "http://admin:18080",
  }));
  console.log(JSON.stringify({ event: "agent.gateway.listening", port: env.port, gatewayId: env.gatewayId, commandQueue: env.queue }));
  const websocket = attachWebSocketTransport(server, env, queue, hub, eventStore, metrics, { replace: (connectionId, channels) => subscriptions.replaceConnection(env.gatewayId, connectionId, channels), remove: (connectionId) => subscriptions.removeConnection(env.gatewayId, connectionId) }, createVibeSocketPolicy(verifier));
