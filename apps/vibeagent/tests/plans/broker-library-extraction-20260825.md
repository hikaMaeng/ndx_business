...
`apps/agent/src/server`의 전송 계층을 `packages/agent/src/broker`로 옮긴 뒤에도 전달 보장이 이동 전과 동일한지 확인한다. 이 리팩터는 동작 변경을 의도하지 않았으므로 **모든 관측값이 이전 실측과 같아야** 하며, 하나라도 다르면 이동 중 의미가 바뀐 것이다.
...
3. `npm test --workspace agent` — 옮긴 51개 + 기존 11개
...
| 항목 | 기대값 | 근거 |
| --- | --- | --- |
| agent 테스트 | 62/62 (51 이동 + 11 기존) | 이동 전 agent 51 + agent 11 |
| lint | 6 task 성공 | |
