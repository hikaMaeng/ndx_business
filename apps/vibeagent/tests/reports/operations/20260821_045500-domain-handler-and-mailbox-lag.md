...
| Check | Result |
| --- | --- |
| Static `agent/server` handler | `hash.sha256` persisted `command` sequence 1 and `hash.sha256.result` sequence 2 |
...
Run `npm test --workspace agent`, `npm test --workspace agent`, `npm run lint --workspace agent`, then `npm run deploy agent`. The deploy must report the published health probe as HTTP 200. Use unique `domain-handler-proof-*` and `domain-abort-proof-*` transaction keys and remove only those scoped rows after the run.
