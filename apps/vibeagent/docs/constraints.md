...
`agent_result_delivery`는 Worker→result queue, `agent_gateway_delivery`는 Router→Gateway queue handoff ledger다. Router는 Gateway별 ledger row를 만든 뒤에만 result source를 삭제한다. Gateway queue 전송이 실패하면 같은 source를 다시 읽어 `AGENT_MAX_GATEWAY_DELIVERY_ATTEMPTS`(기본 10)까지 재시도하고, 한도를 넘으면 ledger row를 `dead`와 `last_error`로 남긴 뒤 source를 ACK한다. dead row는 자동 삭제하지 않는다. 두 ledger의 delivered row는 `AGENT_RETENTION_DAYS`(기본 30일) 뒤 제거한다. socket receipt는 ephemeral이므로 ledger의 완료는 Gateway queue handoff까지 뜻하며 client는 cursor/eventId dedupe로 재접속을 복구한다. terminal persistence는 immutable event가 기록될 때까지 source를 포기하지 않되 `AGENT_TERMINAL_PERSISTENCE_BACKOFF_MAX_SECONDS`까지 visibility를 지수 backoff한다. event·완료 execution/recipient·cursor와 만료 subscription도 Gateway 기동 및 시간당 정리에서 삭제한다. watermark는 retained event 또는 unexpired cursor가 있을 때만 유지하고, 둘 다 없는 오래된 stream은 정리한다. 만료 `running` execution도 정리 작업이 실패로 바꾸지 않는다. retained PGMQ command가 reclaim할 수 있으므로, 만료 건수는 `expiredExecutionLeases` gauge·경고로 관측하고 terminal 상태 변경은 fenced Worker attempt만 할 수 있다. Gateway는 instance lease를 먼저 얻기 전에는 queue 생성·retention·사용자 요청 endpoint·WebSocket을 시작하지 않는다. standby는 `/health`만 200으로 제공하고 `/ready`와 WebSocket upgrade는 503으로 거절한다. Docker HEALTHCHECK와 Agent deploy 검증은 `/ready`를 사용한다. ownership 뒤에는 same bound server의 handler를 교체하므로 startup port gap이 없다. 종료 owner는 reader를 멈춘 뒤 모든 socket과 HTTP 연결을 닫고 durable subscription 삭제가 끝난 후에만 instance lease를 해제한다. WebSocket `1001` close는 best-effort이며 연결 강제 종료에서는 `1006`도 가능하다. 같은 ID의 Gateway는 live owner가 있는 동안 passive standby로 대기하며, SIGKILL 뒤에는 최대 subscription lease 기간(기본 30초) 후 takeover한다.
