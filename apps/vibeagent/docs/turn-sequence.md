...
- 어떤 fact에 어떤 반응기가 붙는지는 [**반응표**](#반응표--유일하게-고리를-아는-것)가
  정한다. 표는 앱의 설정이고, 반응기도 브로커도 브로커도 그 의미를 모른다.
...
| 이벤트 브로커 | 바이브에이전트 웹백엔드에 내장. 소켓과 로그 tail, 그리고 반응 적재 |
| 이벤트 로그 | `event_store`. append만 되고 지워지지 않는다 |
| └ 반응 적재 | 같은 프로세스, 같은 tail. 반응표가 지목한 큐에 사본을 넣는다. 도메인을 모른다 |
...
화살표가 반응기에서 반응기로 가지 않는다. 전부 로그를 거쳐 브로커가 옮긴다.
...
브로커의 ingress가 `vibe_intake` 로 보낸다. 그 뒤로는 전부 브로커가 옮긴다.

---

## 반응표 — 유일하게 고리를 아는 것

앱의 설정이다([`reactions.ts`](../src/server/reactions.ts)). 브로커는 "이 action이면
이 큐들" 만 본다.
...
이전 문서는 이것을 "브로커 재시작 중 유실" 이라고 적었는데 **과장이었다.** 커서는
DB의 행이므로 평범한 재시작은 아무것도 잃지 않는다 — 멈춘 자리에서 이어간다.

실제 구멍은 훨씬 좁다: **저장된 커서가 없는** 브로커가 로그 끝에서 시작하면 그 앞의
...
브로커의 청소 루프가 주기적으로 그걸 찾아 다시 보낸다. 도메인을 모른다 — 반응표의
