...
try {
  ...
  await step("client-event-frame-sent", async () => {
    ...
    await page.getByLabel("Payload JSON").fill(JSON.stringify({ prompt: "event store verification", simulateDelayMs: 120 }, null, 2));
  ...
  const screenshot = path.join(out, "screenshots", "event-store-cps-flow.png");
  ...
  console.log(`agent-event-store-browser status=passed dir=${out}`);
} catch (error) {
