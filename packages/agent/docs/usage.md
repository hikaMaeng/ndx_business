...
```ts
import { createIngressEvent } from "agent/common";
...
```ts
import { createDerivedDraft } from "agent/common";
import { deterministicEventId } from "agent/server";
...
app은 `agent/broker`에서 조립 대상을 가져오고, 자기 것만 주입한다. 주입 지점은 두 곳이다.

```ts
import { createApp, createWorkerPool, readEnv } from "agent/broker";
...
import { executeHandler } from "agent/server";
