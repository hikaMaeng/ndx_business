...
`createWorkerServer`는 두 형태 중 하나를 받는다. 잘못 고르면 조용히 동시성이 막힌다.

| 핸들러가 하는 일 | 넘기는 것 | 이유 |
| --- | --- | --- |
| **계산한다** (해싱·파싱·압축) | `worker: URL` | worker thread가 이벤트 루프를 지킨다 |
| **기다린다** (추론·자식 프로세스·DB) | `execute: fn` | CPU를 안 쓰므로 스레드는 이득 없이 동시성만 `cpus × 2`로 묶는다 |

기다리는 핸들러를 스레드에 올리면, 48코어 장비에서도 동시 세션이 96개로 막히고 대기 중인 턴 하나가 V8 isolate를 통째로 점유한다. 인라인 모드의 상한(`maxConcurrent`)은 코어 수가 아니라 **하류가 감당할 수 있는 양**으로 정한다.
...
브라우저가 클라이언트가 되려면 **브라우저가 직접 닿을 수 있는 포트**가 필요하다. 두 가지가 그렇다.

| 대상 | 왜 노출해야 하나 |
| --- | --- |
| 웹 백엔드 (HTTP) | 클라이언트 번들과 로그인·`me` 요청 |
| **이벤트 브로커 (WebSocket)** | **브라우저가 브로커와 직접 이벤트를 주고받는다** |

두 번째를 놓치기 쉽다. 브로커를 내부 전용으로 두면 화면은 뜨는데 소켓만 안 붙는다.

이 배포는 둘을 **한 프로세스에 co-host**하므로 포트 하나면 된다. 같은 origin이라 CORS도 없고 소켓 URL을 따로 알려 줄 필요도 없다.

```yaml
vibeagent:            # 웹 백엔드 + 이벤트 브로커
  ports: ["18081:18081"]     # 브라우저가 닿아야 한다
vibeagent-worker:     # 워커 서버
  # 포트 없음 — 클라이언트와 직접 통신하지 않는다
vibeagent-router:     # 브로커의 일부
  # 포트 없음 — PGMQ만 본다
```

둘을 분리해 배포한다면 **브로커에도 별도 노출 포트가 필요하고**, 클라이언트에 그 origin을 알려 줘야 하며, CORS와 소켓 origin을 함께 풀어야 한다. 워커와 라우터는 어느 구성에서도 노출하지 않는다 — 브라우저가 그들과 직접 말할 일이 없다.
