...
test("router fans one result into only the subscribed Gateway queues", async () => {
  ...
  const loop = startResultRouter({
    ...
    outbox: { record: async (_event: EventEnvelope, targets: Array<{ gatewayId: string }>) => { recorded.push(...targets.map((target) => target.gatewayId)); }, pending: async (_eventId: string, gatewayIds: string[]) => gatewayIds, delivered: async (_eventId: string, gatewayId: string) => { delivered.push(gatewayId); return true; }, failed: async () => "retry" } as never,
    metrics: { increment: () => undefined } as never, visibilitySeconds: 60, pollSeconds: 1, maxInFlight: 2, maxDeliveryReads: 5, maxGatewayDeliveryAttempts: 3,
  });
...
test("router retains an unmatched result, then archives it after the delivery-read budget", async () => {
  ...
  const loop = startResultRouter({
    ...
    outbox: { record: async () => undefined, pending: async () => [], delivered: async () => true, failed: async () => "retry" } as never,
    metrics: { increment: () => undefined } as never, visibilitySeconds: 60, pollSeconds: 1, maxInFlight: 2, maxDeliveryReads: 5, maxGatewayDeliveryAttempts: 3,
  });
  await new Promise((resolve) => setTimeout(resolve, 15)); loop.stop();
...
test("router acknowledges a result only after a Gateway handoff becomes terminally dead", async () => {
  let read = false; const deleted: string[] = []; const failures: Array<[string, number]> = []; const metrics: string[] = [];
  const loop = startResultRouter({
    queue: { read: async () => read ? await new Promise<never>(() => undefined) : (read = true, [{ id: "45", event, headers: null, readCount: 1 }]), ensure: async () => undefined, send: async () => { throw new Error("queue unavailable"); }, delete: async (_queue: string, id: string) => { deleted.push(id); } } as never,
    resultQueue: "agent_results", gatewayQueuePrefix: "agent_gateway_", subscriptions: { gatewaysFor: async () => ["gateway-a"] } as never,
    outbox: { record: async () => undefined, pending: async () => ["gateway-a"], delivered: async () => true, failed: async (_eventId: string, _gatewayId: string, maxAttempts: number) => { failures.push(["gateway-a", maxAttempts]); return "dead"; } } as never,
    metrics: { increment: (name: string) => { metrics.push(name); } } as never, visibilitySeconds: 60, pollSeconds: 1, maxInFlight: 1, maxDeliveryReads: 5, maxGatewayDeliveryAttempts: 3,
  });
  await new Promise((resolve) => setTimeout(resolve, 15)); loop.stop();
  assert.deepEqual(failures, [["gateway-a", 3]]);
  assert.deepEqual(deleted, ["45"]);
  assert.ok(metrics.includes("gatewayDeliveryDeadLetters"));
});
