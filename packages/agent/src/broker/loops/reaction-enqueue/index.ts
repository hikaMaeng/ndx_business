 ...
 */
export function startReactionEnqueue(input: {
  name: string;
  ...
  reconcileMs?: number;
  /**
   * Decides whether this process is the one that dispatches.
   *
   * Absent means "always", which is right for a single broker. Where several
   * processes each run this loop — one per broker replica — the lease is what
   * stops them dispatching the same facts and overwriting each other's cursor.
   */
  lease?: { held(): Promise<boolean> };
   ...
   *
   * At the end, not the beginning. A broker that started from zero would
  ...
  const seed = async (): Promise<Record<string, string>> => {
    ...
    console.log(JSON.stringify({ event: "broker.reaction.enqueue.seeded", enqueue: input.name, streams: Object.keys(now).length }));
    return now;
  };
  ...
  const drain = async (): Promise<boolean> => {
    ...
    input.metrics.increment("reactionEnqueueReads");
    ...
    for (const fact of batch.events) {
      for (const queueName of input.table[fact.action] ?? []) {
        ...
        input.metrics.increment("reactionEnqueueSends");
      }
   ...
   * broker that starts with no saved position begins at the end of the log
  ...
  const reconcile = async (): Promise<void> => {
    ...
    for (const [identity, { fact, queueName }] of wanted) {
      ...
      await input.queue.send(queueName, work(fact, queueName));
      input.metrics.increment("reactionEnqueueRecovered");
      resent += 1;
    }
    if (resent) {
      console.log(JSON.stringify({ event: "broker.reaction.enqueue.recovered", enqueue: input.name, resent, examined: wanted.size }));
    }
  };
  ...
  const run = async (): Promise<void> => {
    ...
    while (!stopped) {
      try {
        if (input.lease && !(await input.lease.held())) { await wait(input.pollMs); continue; }
        const caughtUp = await drain();
        backoffMs = 100;
        if (caughtUp) await notifier.wait(input.pollMs);
      } catch (error) {
        input.metrics.increment("reactionEnqueueFailures");
        console.error(JSON.stringify({ event: "broker.reaction.enqueue.retry", enqueue: input.name, backoffMs, error: error instanceof Error ? error.message : String(error) }));
  ...
  const sweep = async (): Promise<void> => {
    while (!stopped && sweepMs > 0) {
      await wait(sweepMs);
      if (stopped) return;
      if (input.lease && !(await input.lease.held())) continue;
      try { await reconcile(); }
      catch (error) {
        input.metrics.increment("reactionEnqueueFailures");
        console.error(JSON.stringify({ event: "broker.reaction.reconcile.failed", enqueue: input.name, error: error instanceof Error ? error.message : String(error) }));
      }
    }
  };
