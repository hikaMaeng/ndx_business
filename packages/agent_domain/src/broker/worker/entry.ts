...
export type WorkerExecute = (event: EventEnvelope, signal: AbortSignal) => Promise<unknown>;
...
async function execute(executeHandler: WorkerExecute, event: EventEnvelope, controller: AbortController): Promise<unknown> {
...
export function startWorkerEntry(executeHandler: WorkerExecute): void {
  ...
  parentPort?.on("message", (message: { type: "run" | "abort"; id: string; event?: EventEnvelope }) => {
    ...
    void (async () => {
      try { parentPort?.postMessage({ id: message.id, ok: true, value: await execute(executeHandler, message.event!, controller) }); }
